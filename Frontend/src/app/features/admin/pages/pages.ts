...
export class PagesComponent implements OnInit {
  ...
  applyTemplate(templateId: string): void {
    ...
    if (template && typeof template.buildSections === 'function') {
      // New-style template: generates real sections/blocks data
      this.clientData = {
        schemaVersion: '1.0',
        globalTheme: { primaryColor: '#4f46e5', fontFamily: 'Inter' },
        sections: template.buildSections()
      };
    } else {
      // Legacy HTML-string templates or blank: use default
      this.resetDefaultDemoData();
    }
